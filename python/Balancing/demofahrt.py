...
async def main(right, left=math.nan, time = 3.0, init = True):
    ...
    servo_1 = streams[1]
    await servo_1.command(b"d pos nan "+str(right).encode()+b" nan v0.5 a200 t"+str(time).encode())
    #move the left motor
    servo_2 = streams[2]
    await servo_2.command(b"d pos nan "+str(left).encode()+b" nan v0.5 a200 t"+str(time).encode())
